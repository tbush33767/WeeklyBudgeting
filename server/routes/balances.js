...
router.get('/:weekStart', async (req, res) => {
  try {
    ...
    const balance = await db.prepare('SELECT * FROM week_balances WHERE week_start = ?').get(weekStart);
...
router.put('/:weekStart', async (req, res) => {
  try {
    ...
    const result = await stmt.run(weekStart, actual_balance, actual_balance);
    const updated = result.rows?.[0] || { week_start: weekStart, actual_balance };
...
router.delete('/:weekStart', async (req, res) => {
  try {
    ...
    await db.prepare('DELETE FROM week_balances WHERE week_start = ?').run(weekStart);
